...
class ChatService:
    ...
    def render_ovz_faq_answer(self, q_norm: str) -> str:
        if "льгот" in q_norm:
            return (
                "Для ряда категорий инвалидов и лиц с ОВЗ может действовать преимущественное право при прочих равных условиях конкурса. "
                "Основание подтверждают документами.\n\n"
                "Если на специальности есть вступительные испытания и нужны специальные условия, это указывают в заявлении. "
                "К заявлению прикладывают подтверждение: документ об инвалидности, заключение ЦПМПК или ИПРА.\n\n"
                "Колледжи Москвы адаптированы для обучения ребят с особыми образовательными потребностями. Конкретные адаптированные программы, доступная среда и формат сопровождения зависят от колледжа и выбранной профессии.\n\n"
                "Если есть свидетельство об обучении, а не аттестат, нужно смотреть программы профессионального обучения, включая проект «Профессиональное обучение без границ»: https://pobg.mcrpo.ru/ovz/\n\n"
                "Приёмная комиссия выбранного колледжа подскажет точный перечень документов и доступные условия обучения."
            )
        return (
            "Если есть аттестат и поступление идёт на программу СПО, заявление подают через mos.ru.\n\n"
            "Если на выбранной специальности есть вступительные испытания и нужны специальные условия, это указывают в заявлении. "
            "К заявлению прикладывают подтверждение необходимости условий: документ об инвалидности, заключение ЦПМПК или ИПРА. Оригиналы документов предоставляют в колледж на этапе испытаний.\n\n"
            "Колледжи Москвы адаптированы для обучения ребят с особыми образовательными потребностями. Конкретные адаптированные программы, доступная среда и формат сопровождения зависят от колледжа и выбранной профессии.\n\n"
            "Если у поступающего есть свидетельство об обучении, а не аттестат, нужно смотреть программы профессионального обучения. В Москве есть проект «Профессиональное обучение без границ»: https://pobg.mcrpo.ru/ovz/\n\n"
            "Приёмная комиссия выбранного колледжа подскажет точный перечень документов и доступные условия обучения."
        )
    ...
    def render_common_faq_answer(self, user_query: str) -> str | None:
        ...
        family_svo_context = any(x in q for x in ["вдова", "вдовец", "ребенок участника", "дети участников", "сын участника", "дочь участника"]) and any(
            x in q for x in ["сво", "спецоперац", "мобилиз", "добровол", "контрактник", "боевых действий", "военнослуж"]
        )
        if direct_svo_context or family_svo_context:
            return (
    ...
    def pick_script_answer(self, decision: RouterDecision, user_query: str) -> str:
        ...
        variants: dict[str, list[str]] = {
            "greeting": [
                "Привет! Я - Амби, AI-амбассадор колледжей Москвы, помогаю выбрать профессию и ознакомиться с поступлением в колледжи Москвы. Давай выберем, с чего начать.",
                "Привет! Я - Амби, AI-амбассадор профессий. Можешь спросить про профессию, колледж, поступление или документы.",
                "Привет! Я - Амби, AI-помощник поступления в колледжи Москвы. Давай разберёмся, какое направление тебе подойдёт.",
            ],
            "intro": [
                "Я - Амби, AI-амбассадор колледжей Москвы. Помогаю выбрать профессию, подобрать колледжи и разобраться с поступлением.",
                "Меня зовут Амби. Я AI-помощник поступления в колледжи Москвы: подбираю варианты, объясняю специальности и отвечаю по базе.",
            ],
            "creator": [
