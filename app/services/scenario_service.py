...
ADMISSION_TOPICS: list[tuple[str, str, str]] = [
    ...
    ("benefits", "Льготы при поступлении", "льготы при поступлении в колледж Москвы первоочередное преимущественное право"),
    ("foreigners", "Поступление иностранцев", "иностранные граждане поступление в колледж Москвы документы"),
...
INTEREST_KEYWORDS: list[tuple[str, tuple[str, ...], str]] = [
    ...
    ("it", ("матем", "информ", "код", "программ", "игр", "компьют", "данн", "нейро", "хак", "пентест", "кибер", "сети", "админ"), "интерес к компьютерам, логике и технологиям"),
    ...
    ("production", ("промышлен", "производ", "станок", "станк", "мехатрон", "робот", "автоматиз", "электрон", "машиностро", "металл", "полимер", "оборудован", "бпла", "аддитив", "технолог", "свар"), "интерес к производству, оборудованию, робототехнике и работе с техникой"),
    ("construction", ("стро", "архит", "инжен", "черт", "проектир", "монтаж", "электромонтаж"), "интерес к технике, инженерии и реальным объектам"),
...
PRODUCTION_SPECIALTY_MARKERS = (
    ...
    "монтаж техническое обслуживание и ремонт электронных",
...
class ScenarioService:
    ...
    def production_specialty_entries(self, db: Session) -> list[dict[str, Any]]:
        ...
        for doc in docs:
            ...
            if not any(marker in haystack for marker in PRODUCTION_SPECIALTY_MARKERS):
                continue
    ...
    def admission_topic_from_text(self, text: str) -> str | None:
        ...
        if any(marker in q for marker in ["приемная кампания", "приемная компания", "пк 2026", "2026 27"]):
            return "campaign_2026_2027"
        if self.has_svo_priority_query(text):
            return "svo_priority"
        if any(marker in q for marker in ["правила приема", "правила поступ", "порядок приема"]):
            return "rules_2026"
        if any(marker in q for marker in ["иностран", "гражданств", "соотечественник"]):
            return "foreigners"
        if self.has_general_benefit_query(text) or any(marker in q for marker in ["льгот", "преимуществ", "первоочеред"]):
            return "benefits"
    ...
    def render_admission_topic_answer(self, topic: str | None, state: dict[str, Any]) -> str | None:
        _ = state
        answers = {
            "campaign_2026_2027": (
                ...
                "- для московских выпускников часть сведений может подтянуться автоматически из личного кабинета;\n"
                "- если по специальности есть вступительные испытания, их проходят по расписанию колледжа, а результат обычно учитывается для этой специальности в рамках заявлений.\n\n"
                "Полезная логика такая: сначала выбрать направление и программы, затем проверить сроки, документы и наличие вступительных испытаний."
            ),
            "application": (
                "Заявление на поступление подают электронно через mos.ru.\n\n"
                "Основные шаги:\n"
                "1. Авторизоваться или зарегистрироваться на mos.ru.\n"
                "2. Проверить личный кабинет: паспортные данные, СНИЛС, сведения об образовании.\n"
                "3. Открыть услугу «Запись в колледж» в разделе образования.\n"
                "4. Выбрать форму обучения, колледж и программу, расставить приоритеты.\n"
                "5. Сохранить и отправить заявление.\n\n"
                "Если сведения неполные или недостоверные, заявление могут вернуть на исправление, поэтому перед отправкой лучше внимательно сверить данные."
            ),
            "documents": (
                "Для заявления обычно нужны сведения о поступающем, документе личности, СНИЛС и документе об образовании.\n\n"
                "Для московских выпускников 2025 и 2026 годов часть данных может заполниться автоматически, если она есть в личном кабинете mos.ru. "
                "Для других категорий поступающих могут понадобиться сканы или реквизиты документов.\n\n"
                "Отдельно прикладывают подтверждения, если есть льгота, ОВЗ/инвалидность, индивидуальные достижения, иностранный документ об образовании или другая особая ситуация. "
                "Главное правило: сначала заполнить базовые сведения, затем добавить подтверждающие документы именно под свою категорию."
            ),
            "deadlines": (
                "По локальной FAQ-базе приём заявлений через mos.ru начинается 26 июня 2026 года.\n\n"
                "Ключевые сроки:\n"
                "- московские выпускники 9 класса 2025 и 2026 годов: подача на бюджет до 26 июля 2026 года;\n"
                "- если нужны вступительные испытания для этой категории: подача до 20 июля 2026 года;\n"
                "- остальные категории на базе 9 и 11 классов: подача до 15 августа 2026 года;\n"
                "- если нужны вступительные испытания для остальных категорий: подача до 10 августа 2026 года.\n\n"
            ...
            "foreigners": (
                ...
                "Общая схема такая:\n"
                "- если есть международный договор или другое основание, может быть доступно обучение на бюджете;\n"
                "- для отдельных стран и ситуаций могут понадобиться нотариальный перевод, легализация документа об образовании или подтверждение статуса соотечественника;\n"
                "- если основания для бюджета нет, обычно остаётся внебюджетная форма;\n"
                "- иностранный документ об образовании нужно проверять отдельно: иногда требуется апостиль, консульская легализация или признание документа.\n\n"
                "Самое важное — заранее подготовить документы об образовании и перевод, потому что именно они чаще всего влияют на срок подачи."
            ),
            "army": (
                "Отсрочка от армии обычно связана с очным обучением и соблюдением условий законодательства.\n\n"
                "В практическом смысле нужно помнить:\n"
                "- сама подача заявления в колледж ещё не является отсрочкой;\n"
                "- основание появляется после зачисления и оформления обучения;\n"
                "- важны форма обучения, возраст и предыдущий образовательный маршрут;\n"
                "- документы об обучении затем используют для воинского учёта.\n\n"
                "Если вопрос срочный, сначала стоит разобраться со статусом поступления: подано заявление, есть ли приказ о зачислении, какая форма обучения."
            ),
            "budget": (
                "Бюджетные места зависят от колледжа, специальности и года приёма.\n\n"
                "Что влияет на конкурс:\n"
                "- приоритеты в заявлении;\n"
            ...
            "rules_2026": (
                "Правила приёма 2026/27 определяют, кто может подать заявление, какие сроки действуют, как учитываются результаты и в каком порядке идёт зачисление.\n\n"
                "В базе есть несколько важных ориентиров:\n"
                "- для поступления в колледжи Москвы заявление оформляется через mos.ru;\n"
                "- для выпускников 9 класса Москвы 2025 и 2026 годов действует отдельная логика приёмной кампании;\n"
        ...
        if answer is None:
            return None
        return self.append_related_admission_topics(answer, topic)
    ...
    def find_specialty_options(self, db: Session, query: str, *, limit: int = 18) -> list[dict[str, Any]]:
        result: list[dict[str, Any]] = []
        seen: set[str] = set()
    ...
    def handle_admission(
        ...
        *,
        ...
        if action_code.startswith("admission_topic:"):
            ...
            return self.answer_admission_question(db, session, state, message, query, top_k=top_k, forced_topic=topic)
    ...
    def answer_admission_question(
        ...
        *,
        ...
        topic = forced_topic or self.admission_topic_from_text(original_message) or self.admission_topic_from_text(query)
        ...
        static_answer = self.render_admission_topic_answer(topic, state)
        if static_answer:
            self.session_service.update_route_state(
                db,
                session,
            ...
            return self.save_direct(
                ...
                static_answer,
                "admission",
                self.admission_suggestions(topic),
            )
