...
Эта заметка объясняет, зачем нужна каждая папка и каждый файл в проекте. Ее можно читать как технический конспект перед защитой, ревью или самостоятельной доработкой.

## 1. Главная Идея

`mosobr_ai` - локальный AI-помощник по колледжам Москвы, поступлению и профориентации.
...
- `faq` - документы, поступление, ОВЗ, отсрочка, mos.ru.
...
- "Расскажи про отсрочку от армии" -> `faq`.
...
Используется для документов, ОВЗ, армии, mos.ru, сроков и похожих тем.
