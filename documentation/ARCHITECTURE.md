...
- начинает новую сессию с выбора `Родитель` / `Абитуриент`;
...
- применяет продуктовые правила: педагогика не ранжирует вокал без музыкального контекста, хакинг/пентест ведут в IT/ИБ/сети, сложные запросы делятся на смысловые блоки.
...
Telegram:
- `/start` показывает краткое описание и кнопки выбора роли;
