...
Путь настраивается через `PUBLIC_CHAT_PATH`. Старый `/api/chat` оставлен только для совместимости; в продакшене его можно выключить через `API_LEGACY_CHAT_ENABLED=false`.
...
`suggestions` возвращаются как массив объектов `{label, action}`. `label` можно показать на кнопке, `action` нужно отправлять в следующий `/ambi/v1/dialog` вместе с `message` и `session_id`. Для старых UI дополнительно есть `suggestion_labels` - список только из текстов кнопок.
...
`/ambi/v1/dialog` умеет обрабатывать сообщения, где есть несколько смыслов. Например запрос про `МПК`, педагогику и преимущества за олимпиады будет разобран на основной блок про колледж/направление и отдельный блок про поступление.

## Базовая Защита

- `API_CORS_ORIGINS` задаёт разрешённые домены через запятую. По умолчанию разрешены только локальные origin.
- `API_TRUST_PROXY_HEADERS=true` нужно включать только за доверенным reverse proxy, который сам выставляет `X-Forwarded-*`.
- `API_RATE_LIMIT_PER_MINUTE` ограничивает частоту POST-запросов к диалогу и управлению сессией, по умолчанию `120`.
- `API_MAX_BODY_BYTES` и `API_MAX_MESSAGE_CHARS` ограничивают размер запроса и сообщения.
- `API_LEGACY_CHAT_ENABLED=false` выключает старые `/api/chat` и `/api/session/*`.
